...
func (f factory) GetConnector(prefix string, data *schema.ResourceData) (interface{}, error) {
	...
	if auth, ok := data.GetOk(prefix + "azure_pipelines_auth.0"); ok {
		auth := auth.(map[string]interface{})
		connector.AzurePipelinesAuth = 1
		if auth["terraform_task_v5"].(bool) {
			connector.AzurePipelinesAuth = 2
		}
	}
...
func (c *Connector) connector() (driver.Connector, error) {
	...
	} else if c.AzurePipelinesAuth == 1 {
		// For use in Azure DevOps Pipelines with AzureCLI@2 or AzurePowerShell@5 tasks
		query.Set("fedauth", "ActiveDirectoryAzurePipelines")
		query.Set("systemtoken", os.Getenv("SYSTEM_ACCESSTOKEN"))
	} else if c.AzurePipelinesAuth == 2 {
		query.Set("fedauth", "ActiveDirectoryAzurePipelines")
		// Get variables set by TerraformTask@5 task
		query.Set("serviceconnectionid", os.Getenv("ARM_OIDC_AZURE_SERVICE_CONNECTION_ID"))
		query.Set("user id", fmt.Sprintf("%s@%s", os.Getenv("ARM_CLIENT_ID"), os.Getenv("ARM_TENANT_ID")))
		query.Set("systemtoken", os.Getenv("ARM_OIDC_REQUEST_TOKEN"))
	} else {
